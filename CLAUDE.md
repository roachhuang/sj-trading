...
- `backtest_stats.json` / `equity_snapshot.json` — the live-vs-backtest daily P&L drift check's inputs. `backtest_stats.json` (committed) is a manual snapshot of the backtested daily-return distribution for the live `GridBot.parameters`, written by `backtest.py`'s `write_stats()` — refresh it on the same cadence as the parameter re-tune above (a stale snapshot after a param change silently compares against the wrong distribution). `equity_snapshot.json` is the bot's own prior-day ending equity, written at every end-of-day exit.
  - **The workflow's persist step must commit this file too, alongside `money.json`** — from 2026-08-14 to 2026-09-05 it wasn't (only `money.json` was in the `git add`), so every checkout reset it to its one manual 2026-08-14 commit while real equity drifted away underneath it, false-triggering the drift check's `sys.exit(1)` on every single scheduled run for days. Fixed 2026-09-05 by adding it to the same `git add`/`git diff --quiet` list as `money.json` — if this file's staging is ever split from `money.json`'s again, expect the same failure mode to return.
